...
int init_server_register(const LoginServerRegInfo *reg_info)
{
    ...
    cetcd_array_init(&addrs, 3);
    cetcd_array_append(&addrs, (void *)g_reg_info.reg_center_addr.c_str());
    // cetcd_array_append(&addrs, (void *)g_reg_info.reg_center_addr2.c_str());
    // cetcd_array_append(&addrs, (void *)g_reg_info.reg_center_addr3.c_str());
	...
	netlib_register_timer(server_register_timer_callback, NULL, g_reg_info.ttl); //timer
}
